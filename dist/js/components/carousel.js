import { findById, clamp, setUrlParam } from '../utils/dom.js';
// ---- Structure carousel --------------------------------------------------------
...
export class GalleryCarousel {
    ...
    renderDots(currentPage) {
        ...
        if (total <= MAX_VISIBLE) {
            for (let i = 0; i < total; i++)
                container.appendChild(makeDot(i));
        }
        else {
            let dots;
            if (currentPage <= 2) {
    ...
    openLightbox(index) {
        ...
        const slide = this.els.track.querySelectorAll('.carousel-slide')[index];
    ...
    loadLightboxImage() {
        const slide = this.els.track.querySelectorAll('.carousel-slide')[this.currentIndex];
        const src = slide?.querySelector('img')?.src ?? '';
        if (!src)
            return;
